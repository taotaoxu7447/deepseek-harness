...
`saveImage()` validates bytes and atomically commits one object before returning its reference. `validateImage()` runs the same admission checks without persisting anything; batch callers validate every member through it before saving any member, so validation rejection leaves no partial objects behind. `readImage()` accepts a reference from an authorized session path and returns bytes only after integrity verification. `readImageById()` rebuilds that reference from the stored bytes when the caller knows only the id — the form the vision admission bridge logs. The service is deliberately retention-neutral: resumed and forked sessions may share objects, so reference-aware garbage collection is deferred rather than tied to any one session's deletion.
...
Source: [`packages/attachment/attachment/src/index.ts:32`](../../packages/attachment/attachment/src/index.ts)
