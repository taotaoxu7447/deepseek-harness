...
**Settings-driven configuration over load-time facts.** `vision-qwen` layers its composition entry under the `vision` user-settings section through `installSettingsSection` (the `llm-deepseek` / `web-search-deepseek` pattern): the provider projects the section per describe, so backend order, model, endpoint, enablement, and key changes reach the next call without a restart, and an unconfigured section parks the provider (`available() === false`) instead of failing the load — the web Plugins settings card is the intended place to finish setup. This trades load-time failure for a clean selection refusal with a pointer at that page, which is the right failure point when the configuration interface is a UI. Serving a namespace to the browser is an explicit decision in `WEB_SETTINGS_NAMESPACES` (`dsh-apiproxy`), so each section must be added there too.

**Default composition, dormant execution.** The base bundle mounts `dsh-vision` and `dsh-vision-qwen`, while the full `standard`, `code`, and `cordis` Agent Presets mount `dsh-tool-vision`. The Web Plugins page therefore always exposes the `vision` section, and full agents always carry `view_image`; the provider performs no request until the user configures an enabled backend. Minimal stays a fixed two-tool preset and does not mount the consumer.

**Multiple backends in one ordered chain.** The single `vision-qwen` provider projects up to five configured backends from the `vision` section and attempts them in priority order. Each backend has its own id, endpoint, model, credential reference, and enablement; exhausting one backend's attempt budget falls through to the next. The provider package stays OpenAI-compatible-generic (the Qwen name is its origin, not its protocol), while the capability service retains its provider-selection errors for deployments that compose more than one provider plugin.
