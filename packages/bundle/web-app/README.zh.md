...
- **无构建记录的部署永不报告 stale**：没有 `.dsh-build/client-build-environment.json` 时健康路由应答 `builtAt: null`；staleness 信号只面向 checkout 构建的服务器，安装版永远不会被打扰。
