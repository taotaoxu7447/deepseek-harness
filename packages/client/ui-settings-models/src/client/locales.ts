...
export const en = {
  ...
  streamIdleTimeoutHint: 'Idle limit for one provider stream read; a local model that thinks long between chunks needs a larger value. Blank inherits the default below.',
  streamIdleTimeoutInvalid: 'Enter a positive number of seconds, at most 2147483.',
  models: 'Models',
  ...
  modelInputModalitiesHint: 'What this model accepts. text is always on; turn image on to declare a vision model.',
  modelReasoningEffortsPlaceholder: 'off, high, max',
  modelReasoningEffortsHint: 'Toggle levels above; rename what is sent with level=wire. Empty inherits the catalog; false disables reasoning.',
  modelReasoningEffortsInvalid: 'Use known levels (off, minimal, low, medium, high, xhigh, max); every level except off needs a wire value.',
  modelEffortLevels: 'Levels',
  modelEffortPresets: 'Presets',
  'effortPreset.deepseek-v4': 'DeepSeek V4 (off/high/max)',
  'effortPreset.openai-grades': 'OpenAI (none–max)',
  'effortPreset.anthropic-claude': 'Anthropic Claude (low–max)',
  'effortPreset.all-levels': 'All levels',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  streamIdleTimeoutHint: '单次流读取的空闲上限；本地模型长时间思考不出字时需要调大。留空继承下方默认值。',
  streamIdleTimeoutInvalid: '请输入正数秒，最大 2147483。',
  models: '模型目录',
  ...
  modelInputModalitiesHint: '模型接受的输入类型。text 始终开启；视觉模型请打开 image。',
  modelReasoningEffortsPlaceholder: 'off, high, max',
  modelReasoningEffortsHint: '点击上方档位开关；需要改名发送时写成 档位=线上值。留空继承目录；false 表示不支持推理。',
  modelReasoningEffortsInvalid: '请使用已知档位（off、minimal、low、medium、high、xhigh、max）；除 off 外每个档位都要有线上值。',
  modelEffortLevels: '档位',
  modelEffortPresets: '预设',
  'effortPreset.deepseek-v4': 'DeepSeek V4（off/high/max）',
  'effortPreset.openai-grades': 'OpenAI（none–max）',
  'effortPreset.anthropic-claude': 'Anthropic Claude（low–max）',
  'effortPreset.all-levels': '全部档位',
