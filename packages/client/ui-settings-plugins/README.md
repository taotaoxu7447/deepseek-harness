...
The first batch covers the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), the DeepSeek search provider (`web-search-deepseek`), and the Qwen vision provider (`vision-qwen`).
