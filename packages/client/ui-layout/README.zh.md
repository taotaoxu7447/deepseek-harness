...
框架还持有窗口的远程标签条：`ctx.layout.openRemoteTab({ id, label, url })` 把一条隧道化的远程 UI 摆到本地会话旁；只要存在远程标签页，列上方就出现标签栏，切换标签会把对应的 iframe 换入框架——本地框架保持挂载仅被隐藏，进行中的对话不会因一次标签切换丢失任何东西；每个已摆入的远程都在自己的 iframe 中持续运行，iframe 以 `id@url` 为键，因此一次把隧道端口挪走的重连只重挂载那一个标签页。关闭当前标签会改为摆入最新的幸存者，没有幸存者时回到本地视图；`activateRemoteTab`、`showLocalTab` 与 `closeRemoteTab` 补齐这组服务接口。标签条文案走本包的 `layout` locale 命名空间。与布局 store 的其余部分一样，远程标签页是瞬态的：重载即丢弃。
...
- **面板几何信息与远程标签页都是瞬时状态**：重新加载会恢复侧边栏默认值、使详情栏保持关闭，并丢弃所有已摆入的远程标签页；在不同会话 id 之间切换同样会关闭详情栏，并忘记拖动后的宽度，而未选中表面会以零宽度渲染详情栏，但不会修改几何信息。
